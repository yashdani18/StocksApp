...
df_trend_response, dict_periodic_data, hist_df = get_trend('TCS', 'NSE', 'daily', int(str(datetime.now()).split(' ')[0].split('-')[2]), int(str(datetime.now()).split(' ')[0].split('-')[1]), int(str(datetime.now()).split(' ')[0].split('-')[0]) - 1)
