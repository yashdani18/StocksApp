...
def get_trend(p_ticker, p_exchange, p_interval, p_date, p_month, p_year):
    ...
    for datapoint in meta_data_points:
        ...
        # print(datapoint, price_open, (price_close - price_open) / price_close * 100, (price_close - price_open) /
        # price_open * 100)
...
f = open('../data/data_companies_portfolio.json')
companies = json.load(f)['companies']
for company in companies:
    print('for', company['symbol'])
    df_trend_response, dict_periodic_data, hist_df = get_trend(company['symbol'], 'NSE', 'weekly',
                                                               int(str(datetime.now()).split(' ')[0].split('-')[2]),
                                                               int(str(datetime.now()).split(' ')[0].split('-')[1]),
                                                               int(str(datetime.now()).split(' ')[0].split('-')[0]) - 1)
    print_dataframe(df_trend_response)
