...
from get.get_jobs_root import get_count_of_documents_from_collection
from get.get_new_jobs import get_all_new_jobs
...
def home():
    options = [
        ...
        },
        {
            ...
            "desc": "Companies that have recently posted new job openings",
            "endpoint": "jobs"
        }
    ]
    form = LoginForm()
...
def company(ticker):
    ...
    val_qr_sales = [float(qr_sales[-5].replace(',', '')), float(qr_sales[-1].replace(',', '')),
                    float(qr_sales[-2].replace(',', ''))]
    rel_qr_sales = ["{:.2f}".format(val_qr_sales[1] / val_qr_sales[0]),
                    dictionary_screener['Rel ' + SCREENER_KEYS.QR_SALES][-1]]
    ...
    news_companies = [
        {
            ...
            'ticker': 'HAPPSTMNDS',
            'function': get_news_from_happiest_minds
        },
        {
            'company': 'Tata Chemicals',
            'ticker': 'TATACHEM',
            'function': get_news_from_tata_chemicals
        },
        {
            'company': 'Tata Consumer',
            'ticker': 'TATACONSUM',
            'function': get_news_from_tata_consumer
        },
        {
            'company': 'Tata Elxsi',
            'ticker': 'TATAELXSI',
            'function': get_news_from_tata_elxsi
        },
        {
            'company': 'Tata Motors',
            'ticker': 'TATAMOTORS',
            'function': get_news_from_tata_motors
        },
        {
            'company': 'Tata Power',
            'ticker': 'TATAPOWER',
            'function': get_news_from_tata_power
        }
        # },
        # {
        #     'company': 'Tata Steel',
        #     'ticker': 'TATASTEEL',
        #     'function': get_news_from_tata_steel()
        # }
    ]
    arr_news = []
    if request.method == 'POST':
        print(request.form['action'])
        company_name = request.form['action']
        for index, news_company in enumerate(news_companies):
            if news_company['company'] == company_name:
                selected_index = index + 1
                arr_news = news_company['function']()
                print(arr_news)
...
def jobs():
    ...
    dict_jobs = get_all_new_jobs()
    new_jobs = dict_jobs['new_jobs']
    all_jobs = dict_jobs['all_jobs']
    len_jobs = len(new_jobs)
    return render_template("jobs.html", new_jobs=new_jobs, len_jobs=len_jobs, all_jobs=all_jobs)
